import { Storage, MemberGroup, Member } from "../../models";
import { groupCollapsed } from "console";

const memberGroups: MemberGroup[] = [
  {
    id: 1,
    name: "Party Planning Committee",
    members: [
      4, 7, 8, 9, 18, 
    ],
  },
  {
    id: 2,
    name: "Dinner Club",
    members: [
      1, 20, 3, 4, 5, 8, 
    ]
  }
]

const members: Member[] = [
  {
    email: "michael.scott@dundermifflin.com",
    facebook: "michaelgaryscott",
    fullName: "Michael Gary Scott",
    id: 1,
    phone: "(123)-456-7891",
    twitter: "@prison_mike"
  },
  {
    email: "jim.halpert@dundermifflin.com",
    facebook: "jameshalpert",
    fullName: "Jim Halpert",
    id: 2,
    phone: "(123)-456-7891",
    twitter: "@gumby_with_hair"
  },
  {
    email: "dwight.schrute@dundermifflin.com",
    facebook: "dwightkschrute",
    fullName: "Dwight Kurt Schrute",
    id: 3,
    phone: "(123)-456-7891",
    twitter: "@schrute_farms"
  },
  {
    email: "pamela.halpert@dundermifflin.com",
    facebook: "pamhalpert",
    fullName: "Pamela Beesley Halpert",
    id: 4,
    phone: "(123)-456-7891",
    twitter: "@pam_halpert"
  },
  {
    email: "andrew.bernard@dundermifflin.com",
    facebook: "andybernard",
    fullName: "Andrew Banes Bernard",
    id: 5,
    phone: "(123)-456-7891",
    twitter: "@here_comes_treble_94"
  },
  {
    email: "kevin.malone@dundermifflin.com",
    facebook: "kevinmalone",
    fullName: "Kevin Malone",
    id: 6,
    phone: "(123)-456-7891",
    twitter: "@kevins_famous_chili"
  },
  {
    email: "oscar.martinez@dundermifflin.com",
    facebook: "oscarmartinez",
    fullName: "Oscar Martinez",
    id: 7,
    phone: "(123)-456-7891",
    twitter: "@actually"
  },
  {
    email: "angela.martin@dundermifflin.com",
    facebook: "angelamartin",
    fullName: "Angela Martin",
    id: 8,
    phone: "(123)-456-7891",
    twitter: "@sprinkles"
  },
  {
    email: "meredith.palmer@dundermifflin.com",
    facebook: "meredithpalmer",
    fullName: "Meredith Palmer",
    id: 9,
    phone: "(123)-456-7891",
    twitter: "@booze"
  },
  {
    email: "creed.bratton@dundermifflin.com",
    facebook: "creedbratton",
    fullName: "Creed Bratton",
    id: 10,
    phone: "(123)-456-7891",
    twitter: "@real_creed_bratton"
  },
  {
    email: "ryan.howard@dundermifflin.com",
    facebook: "ryanhoward",
    fullName: "Ryan Howard",
    id: 11,
    phone: "(123)-456-7891",
    twitter: "@wuphf_daddy"
  },
  {
    email: "kelly.kapoor@dundermifflin.com",
    facebook: "kellykapoor",
    fullName: "Kelly Rajnigandha Kapoor",
    id: 12,
    phone: "(123)-456-7891",
    twitter: "@ryans_girl"
  },
  {
    email: "darryl.philbin@dundermifflin.com",
    facebook: "darrylphilbin",
    fullName: "Darryl Philbin",
    id: 13,
    phone: "(123)-456-7891",
    twitter: "@warehouse_darryl"
  },
  {
    email: "toby.flenderson@dundermifflin.com",
    facebook: "tobyflenderson",
    fullName: "Toby Flenderson",
    id: 14,
    phone: "(123)-456-7891",
    twitter: "@toby"
  },
  {
    email: "gabe.lewis@dundermifflin.com",
    facebook: "gabelewis",
    fullName: "Gabe Lewis",
    id: 15,
    phone: "(123)-456-7891",
    twitter: "@adams_apple"
  },
  {
    email: "erin.hannon@dundermifflin.com",
    facebook: "erinhannon",
    fullName: "Erin Hannon",
    id: 16,
    phone: "(123)-456-7891",
    twitter: "@space_orphan"
  },
  {
    email: "robert.california@dundermifflin.com",
    facebook: "robertcalifornia",
    fullName: "Robert California",
    id: 17,
    phone: "(123)-456-7891",
    twitter: "@lizard_king"
  },
  {
    email: "phyllis.vance@dundermifflin.com",
    facebook: "phyllisvance",
    fullName: "Phyllis Vance",
    id: 18,
    phone: "(123)-456-7891",
    twitter: "@bobs_wife"
  },
  {
    email: "stanley.hudson@dundermifflin.com",
    facebook: "stanleyhudson",
    fullName: "Stanley Hudson",
    id: 19,
    phone: "(123)-456-7891",
    twitter: "@jury_duty"
  },
  {
    email: "jan@serenitybyjan.com",
    facebook: "janlevinson",
    fullName: "Jan Levinson",
    id: 20,
    phone: "(123)-456-7891",
    twitter: "@serenity_by_jan"
  }
]

export class JsonDataStorageProvider implements Storage {
  
  public async getMembers(): Promise<Member[]> {
    return members;
  }
  
  public async getMember(id: number): Promise<Member> {
    const member = members.find(member => member.id == id);
    if (!member) {
      throw new Error(`Member with ID ${id} does not exist`);
    }
    return member;
  }
  
  public async getGroups(): Promise<MemberGroup[]> {
    return memberGroups;
  }
  
  public async getGroup(id: number): Promise<MemberGroup> {
    const group = memberGroups.find(group => group.id == id);
    if (!group) {
      throw new Error(`Member with ID ${id} does not exist`);
    }
    return group;
  }
}
